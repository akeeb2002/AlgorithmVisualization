...
def plot_times(dict_algs, sizes, trials, algs, title, file_name):
    ...
    for alg in algs:
        alg_name = alg[2]
        if alg_name not in dict_algs:
            continue
        d = dict_algs[alg_name]
        x_axis = [j + offset for j in range(len(sizes))]
        y_axis = [d[i] for i in sizes]
        plt.bar(x_axis, y_axis, width=width, alpha=0.75, label=alg_name)
        offset += width
        plt.xticks([j + offset-width/2 for j in range(len(sizes))], [str(size) for size in sizes])
    plt.title(title)
    plt.legend()
    plt.title("Runtime of Search Structures(Build & Search)")
