...
def hash_probing_build(arr):  # good
    size = len(arr) * 2
    ...
    for key in arr:
        ...
        if hash_table[hash_key] is None:
            ...
            i = 0
            while hash_table[hash_key] is not None:  # collision  # is not None
                i += 1
...
def hash_chaining_search(hash_table, arr):  # good
    size = len(hash_table)
    found = [False] * len(arr)
...
def hash_probing_search(hash_table, arr):  # good
    size = len(hash_table)
    found = [False] * len(arr)
    print()
    for j in range(len(arr)):
...
def main():
    ...
    print('Akeeb Khandakar\n')
    print("Output of Mini Test:\n")
    mini_test()
    print("\nOutput of Big Test:\n")
    big_test()
    print("\nEND")
