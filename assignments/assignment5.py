...
def f8_matrix_multiplication_int_mult(f, n):
    return 1 if n == 1 else 7 * ff(f, int(n/2)) + n^2


def f9_exponential_int_mult(f, n):
    return 1 if n == 1 else ff(f, int(n/2)) + 1
...
def evaluate_recurrences():
    funcs = [f0_fibonacci, f1_merge_sort, f2_linear_search, f3_binary_search,
             ...
             f8_matrix_multiplication_int_mult, f9_exponential_int_mult]
...
def evaluate_master_theorem():
    examples = [("Binary Search", 1, 2, 0),
                ...
                ("Matrix Multiplication", 8, 3, 1),
                ("Exponential Sort", 1, 2, 0),
                ] 
